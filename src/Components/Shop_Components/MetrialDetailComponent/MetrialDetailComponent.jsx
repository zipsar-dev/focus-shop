...
import { useKit } from "../../../context/KitContext"; // Import Kit context
import Footer from "../../../Components/Common/Footer/Footer";

const MaterialDetailComponent = () => {
  const { id } = useParams();
  const material = products.find((material) => material.keyName === id);
  const { addToCart, removeFromCart, getProductQuantity } =
    useContext(CartContext);
  const { selectedKit, getProductPrice } = useKit(); // Use Kit context instead of hardcoded default
  ...
  // Get current price based on selected kit
  const currentPrice = getProductPrice(material);
  const quantity = getProductQuantity(material.id, selectedKit);
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-4 pt-4">
                  ...
                  <div className="flex items-center border border-gray-300 rounded-full px-3 py-2 w-full sm:w-auto sm:min-w-[120px] justify-between">
                    <button
                      onClick={() => removeFromCart(material.id, selectedKit)}
                      disabled={quantity === 0}
